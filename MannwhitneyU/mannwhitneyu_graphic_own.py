...
def create_window():
    ...
    method_select_asymptotic.grid(row=5,column=0,padx=5,pady=5)
    ...
    method_select_auto.grid(row=5,column=2,padx=5,pady=5)

    

    Calculate_Button = tk.Button(window, 
                text="Calculate",
                command = lambda: mannwhitneyu_calc(entry_a, entry_b, window, mannwhitneyu_method.get()))
    Calculate_Button.grid(row=7,columnspan=3,pady=20 )
    window.bind('<Return>', lambda event: mannwhitneyu_calc(entry_a, entry_b, window, mannwhitneyu_method.get()))
...
def mannwhitneyu_calc(entry_a, entry_b, window, mannwhitneyu_method):
    group_a = [float(x) for x in entry_a.get().split(" ")]
    group_b = [float(x) for x in entry_b.get().split(" ")]
    U1, p = mannwhitneyu(group_a, group_b, method=mannwhitneyu_method)
...
def show_result(window, p, U1, U2):
    ...
    text_Label.grid(row=8, column= 1, pady=10)
    ...
    result_widget.grid(row = 9, column = 1)
